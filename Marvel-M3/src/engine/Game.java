...
public class Game {
	...
	public void move(Direction d) throws NotEnoughResourcesException,
		...
		if (hasEffect(getCurrentChampion(), "Root"))
			throw new UnallowedMovementException(
					"You can not move while being rooted");
		if (getCurrentChampion().getCurrentActionPoints() < 1)
			throw new NotEnoughResourcesException(
					"You need at least one action point to move");
	...
	public void attack(Direction d) throws NotEnoughResourcesException,
		...
		if (hasEffect(getCurrentChampion(), "Disarm"))
			throw new ChampionDisarmedException(
					"Can not attack while being disarmed");
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			if (d == Direction.UP)
				...
				curry++;
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0
					|| curry >= BOARDWIDTH)
				return;
				...
				if (board[currx][curry] instanceof Cover) {
					...
					if (curhp <= 0)
						board[currx][curry] = null;
					getCurrentChampion().setCurrentActionPoints(
							getCurrentChampion().getCurrentActionPoints() - 2);
					return;
					...
					if (firstPlayer.getTeam().contains(getCurrentChampion())
						...
						continue;
					else if (secondPlayer.getTeam().contains(
							getCurrentChampion())
					...
					if (hasEffect(target, "Dodge")) {
						...
						if (r <= 50) {
							curr.setCurrentActionPoints(curr
									.getCurrentActionPoints() - 2);
							return;
						}
					}
	...
	public void castAbility(Ability a) throws NotEnoughResourcesException,
		...
		if (a.getCastArea() == AreaOfEffect.SELFTARGET) {
			...
			if (a instanceof DamagingAbility
					|| (a instanceof CrowdControlAbility && ((CrowdControlAbility) a)
							.getEffect().getType() == EffectType.DEBUFF)) {
				if (firstPlayer.getTeam().contains(getCurrentChampion()))
					team = secondPlayer.getTeam();
				else
					team = firstPlayer.getTeam();
			} else if (a instanceof HealingAbility
					|| (a instanceof CrowdControlAbility && (a instanceof CrowdControlAbility && ((CrowdControlAbility) a)
							.getEffect().getType() == EffectType.BUFF))) {
				if (firstPlayer.getTeam().contains(getCurrentChampion()))
					team = firstPlayer.getTeam();
				else
					team = secondPlayer.getTeam();
			...
			for (Champion c : team) {
				...
				int distance = Math.abs((int) getCurrentChampion()
						.getLocation().getX() - x)
						+ Math.abs((int) getCurrentChampion().getLocation()
								.getY() - y);
	...
	private void validateCastAbility(Ability a)
			throws NotEnoughResourcesException, AbilityUseException {
		if (getCurrentChampion().getMana() < a.getManaCost())
			throw new NotEnoughResourcesException("you need at least "
					+ a.getManaCost() + " mana to cast this ability");
		else if (getCurrentChampion().getCurrentActionPoints() < a
				.getRequiredActionPoints())
			throw new NotEnoughResourcesException("you need at least "
					+ a.getRequiredActionPoints()
					+ " action points to cast this ability");
		else if (hasEffect(getCurrentChampion(), "Silence"))
			throw new AbilityUseException(
					"You can not cast an ability while being silenced");
		else if (a.getCurrentCooldown() > 0)
			throw new AbilityUseException(
					"You can not use an ability while it is in cooldown");
	}
	...
	public void castAbility(Ability a, Direction d)
		...
		ArrayList<Damageable> targets = prepareTargetsFromPoints(a,
				possiblePoints);
	...
	private ArrayList<Damageable> prepareTargetsFromPoints(Ability a,
			...
			if (x >= 0 && x < BOARDHEIGHT && y >= 0 && y < BOARDWIDTH) {
				...
				if (o != null) {
					if (o instanceof Cover) {
						...
						boolean friendly = ((firstPlayer.getTeam().contains(
								getCurrentChampion()) && firstPlayer.getTeam()
								.contains(o)) || ((secondPlayer.getTeam()
								.contains(getCurrentChampion()) && secondPlayer
								.getTeam().contains(o)))) ? true : false;
						if (a instanceof HealingAbility && friendly)
							...
							if (hasEffect(c, "Shield")) {
								...
								&& ((CrowdControlAbility) a).getEffect()
										.getType() == EffectType.DEBUFF
								&& !friendly)
							targets.add((Damageable) o);
						else if (a instanceof CrowdControlAbility
								&& ((CrowdControlAbility) a).getEffect()
										.getType() == EffectType.BUFF
								&& friendly)
							targets.add((Damageable) o);
					}
				}
	...
	public void castAbility(Ability a, int x, int y)
		...
		if (board[x][y] == null)
			throw new InvalidTargetException(
					"You can not cast an ability on an empty cell");
		int distance = Math.abs((int) getCurrentChampion().getLocation().getX()
				- x)
				+ Math.abs((int) getCurrentChampion().getLocation().getY() - y);
		if (distance > a.getCastRange())
			throw new AbilityUseException(
					"Target out of the ability's cast range");
		...
		if (board[x][y] instanceof Champion) {
			...
			boolean friendly = ((firstPlayer.getTeam().contains(
					getCurrentChampion()) && firstPlayer.getTeam().contains(
					target)) || ((secondPlayer.getTeam().contains(
					getCurrentChampion()) && secondPlayer.getTeam().contains(
					target)))) ? true : false;
			if (friendly && a instanceof DamagingAbility)
				throw new InvalidTargetException(
						"Can not cast damaging ability on friendly targets");
			if (friendly
					&& a instanceof CrowdControlAbility
					&& ((CrowdControlAbility) a).getEffect().getType() == EffectType.DEBUFF)
				throw new InvalidTargetException(
						"Can not debuff friendly targets");
			if (!friendly && a instanceof HealingAbility)
				throw new InvalidTargetException(
						"Can not cast healing ability on enemy targets");
			if (!friendly
					&& a instanceof CrowdControlAbility
	...
	public void useLeaderAbility() throws LeaderNotCurrentException,
		...
		if (getCurrentChampion() != firstPlayer.getLeader()
				&& getCurrentChampion() != secondPlayer.getLeader())
			throw new LeaderNotCurrentException(
					"The current champion is not a leader");
		if (getCurrentChampion() == firstPlayer.getLeader()
				&& firstLeaderAbilityUsed)
			throw new LeaderAbilityAlreadyUsedException(
					"This leader already used his ability");
		if (getCurrentChampion() == secondPlayer.getLeader()
				&& secondLeaderAbilityUsed)
			throw new LeaderAbilityAlreadyUsedException(
					"This leader already used his ability");
		ArrayList<Champion> targets = new ArrayList<Champion>();
		if (getCurrentChampion() instanceof Hero) {
			ArrayList<Champion> team = getCurrentChampion() == firstPlayer
					.getLeader() ? firstPlayer.getTeam() : secondPlayer
					.getTeam();
			targets.add(getCurrentChampion());
			for (Champion c : team)
				targets.add(c);
			...
			ArrayList<Champion> enemies = getCurrentChampion() == firstPlayer
					.getLeader() ? secondPlayer.getTeam() : firstPlayer
					.getTeam();
	...
	private void cleanup(ArrayList<Damageable> targets) {
		for (Damageable c : targets) {
			if (c.getCurrentHP() == 0) {
				board[(int) c.getLocation().getX()][(int) c.getLocation()
						.getY()] = null;
